...
// Axis-level prose — describes the spectrum itself (used for weak/no-preference rows)
...
function DimRow({ entry, rank, accentColor }: {
  ...
  const oppAvg = entry.oppSampleFilms.length > 0
    ? entry.oppSampleFilms.reduce((s, f) => s + f.stars, 0) / entry.oppSampleFilms.length
    : null

  // Film ordering: strong/moderate → dom=highest, opp=lowest; weak → middle slice of each
  ...
  // Prose: tier-aware, personalised — every tier explains what the poles actually are
  const prose = signalTier === 'strong'
    // Strong: what you love about dom pole + what doesn't land on the other end
    ? `${entry.description} ${entry.oppNegativeDescription}`
    : signalTier === 'moderate'
    // Moderate: explains the dom pole, notes it's a real but partial lean
    ? `${entry.description} Your ratings pull this direction, though not as strongly as your defining preferences.`
    // Weak: equal ratings + explains what both poles actually are
    : `Your ratings don't clearly separate ${entry.label} from ${entry.oppLabel}. ${DIM_SPECTRUM_PROSE[entry.dimKey] ?? ''}`

  const tierBg = signalTier === 'strong'
    ? 'rgba(74, 107, 62, 0.04)'
    : signalTier === 'moderate'
    ? 'rgba(190, 150, 60, 0.035)'
    : 'rgba(160, 80, 60, 0.04)'
  const tierBorderColor = signalTier === 'strong'
    ? 'rgba(74, 107, 62, 0.4)'
    : signalTier === 'moderate'
    ? 'rgba(190, 150, 60, 0.45)'
    : 'rgba(160, 80, 60, 0.4)'

  return (
    <div style={{
      display: 'grid',
      gridTemplateColumns: '24px 1fr 200px 1fr',
      gap: 24,
      ...
      {/* Rank */}
      <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--ink-4)', paddingTop: 4, textAlign: 'right' }}>
        {rank}
      </div>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 8 }}>
          <span style={{ fontFamily: 'var(--serif-display)', fontSize: 36, fontWeight: 600, lineHeight: 1, color: 'var(--ink)' }}>
            {entry.letter}
          </span>
          <span style={{ fontFamily: 'var(--serif-display)', fontSize: 15, fontWeight: 500, color: accentColor }}>
            {entry.label}
          </span>
        </div>
      ...
      <div>
        ...
        <p style={{
          ...
          opacity: signalTier === 'strong' ? 1 : signalTier === 'moderate' ? 0.82 : 0.6,
          margin: 0,
        }}>
          {prose}
        </p>
      ...
      {/* Opposite pole (right) — mirrored */}
      <div style={{ textAlign: 'right' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 8, justifyContent: 'flex-end' }}>
          <span style={{ fontFamily: 'var(--serif-display)', fontSize: 15, fontWeight: 500, color: 'var(--ink-4)' }}>
            {entry.oppLabel}
          </span>
          <span style={{ fontFamily: 'var(--serif-display)', fontSize: 36, fontWeight: 600, lineHeight: 1, color: 'var(--ink-3)' }}>
            {entry.oppLetter}
          </span>
        </div>
...
export default function MyTasteCodePage() {
    ...
    <AppShell>
      <div style={{ padding: '40px 64px 100px', maxWidth: 1060, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 36 }}>
          ...
          <a
            href="/api/profile/taste-export"
            download="taste-dimensions-export.csv"
            style={{
              fontFamily: 'var(--mono)', fontSize: 8.5, color: 'var(--ink-4)',
              letterSpacing: '0.08em', textDecoration: 'none',
              border: '0.5px solid var(--paper-edge)', borderRadius: 6,
        ...
        <div style={{ marginBottom: 48 }}>
          ...
          <h1 className="t-display" style={{ fontSize: 46, margin: 0, lineHeight: 1 }}>
            {firstName
              ? <>{firstName}'s <span style={{ fontStyle: 'italic', fontWeight: 300, color: 'var(--s-ink)' }}>dimensions</span>.</>
              : <>your <span style={{ fontStyle: 'italic', fontWeight: 300, color: 'var(--s-ink)' }}>dimensions</span>.</>
            }
          </h1>
          <p style={{ fontFamily: 'var(--serif-italic)', fontStyle: 'italic', fontSize: 13, color: 'var(--ink-4)', marginTop: 12, lineHeight: 1.6 }}>
            all 12 cinematic dimensions, ranked by signal strength — how strongly your ratings pull toward one pole.
          </p>
        </div>
        ...
        {!loading && !tasteCode && (
          <p style={{ fontStyle: 'italic', fontSize: 14, color: 'var(--ink-3)', fontFamily: 'var(--serif-italic)' }}>
            rate more films to generate your taste code.
          </p>
        )}
