...
create policy "own taste tags" on user_taste_tags
  for all using (auth.uid() = user_id);

create policy "own dismissed" on dismissed_recs
  for all using (auth.uid() = user_id);

-- films are public (not personal data)
alter table films enable row level security;
create policy "films readable by all" on films for select using (true);
create policy "films writable by service" on films for all using (auth.role() = 'service_role');
